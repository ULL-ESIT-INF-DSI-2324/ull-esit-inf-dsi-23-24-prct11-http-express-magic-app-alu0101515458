...
export class CardCollection {
  ...
  /**
   * Función que añade una nueva carta a la colección.
   * Utiliza promesas para añadir la carta.
   * @param {Card} card - Carta a añadir.
   * @returns {Promise<void>}
   */
  public addCard(card: Card): Promise<void> {
    // Devolvemos una promesa para poder esperar a que se añada la carta.
    return new Promise((resolve, reject) => {
      // Comprobamos si la carta ya existe en la colección
      if (this.collection.some((c) => c.id === card.id)) {
        console.log(
          chalk.red(
            `La carta con ID ${card.id} ya existe en la colección de ${this.user}.`,
          ),
        );
        reject(`La carta con ID ${card.id} ya existe.`);
        return;
      }

      // Añadimos la carta a la colección y la guardamos en un fichero
      this.collection.push(card);
      // Utilizamos promesas para escribir el fichero.
      fs.promises
        .writeFile(this.getCardFilePath(card.id), JSON.stringify(card, null, 2))
        .then(() => {
          // Si se ha guardado correctamente, resolvemos la promesa.
          console.log(
            chalk.green(`Nueva carta añadida a la colección de ${this.user}.`),
          );
          resolve();
        })
        .catch((err) => {
          // Si ha habido un error, lo mostramos y rechazamos la promesa.
          console.error(chalk.red(`Error al añadir la carta: ${err}`));
          reject(`Error al añadir la carta: ${err}`);
        });
    });
  }
  ...
  public updateCard(updatedCard: Card): void {
    ...
    if (cardIndex === -1) {
      console.log(
        chalk.red(
          `La carta con ID ${updatedCard.id} no existe en la colección de ${this.user}.`,
        ),
      );
    } else {
      this.collection[cardIndex] = updatedCard;
      fs.writeFileSync(
        this.getCardFilePath(updatedCard.id),
        JSON.stringify(updatedCard, null, 2),
      );
      console.log(
        chalk.green(`Carta actualizada en la colección de ${this.user}.`),
      );
    }
  }
  ...
  /**
   * Función que elimina una carta de la colección.
   * Utiliza promesas para eliminar la carta.
   * @param {number} id - ID de la carta a eliminar.
   * @returns {Promise<void>}
   */
  public removeCard(id: number): Promise<void> {
    // Devolvemos una promesa para poder esperar a que se elimine la carta.
    return new Promise((resolve, reject) => {
      ...
      if (cardIndex === -1) {
        console.log(
          chalk.red(
            `La carta con ID ${id} no existe en la colección de ${this.user}.`,
          ),
        );
        reject(`La carta con ID ${id} no existe.`);
      }
      // Eliminamos la carta de la colección y del fichero
      this.collection.splice(cardIndex, 1);
      // Utilizamos promesas para eliminar el fichero.
      fs.promises
        .unlink(this.getCardFilePath(id))
        .then(() => {
          // Si se ha eliminado correctamente, resolvemos la promesa.
          console.log(
            chalk.green(`Carta eliminada de la colección de ${this.user}.`),
          );
          resolve();
        })
        .catch((err) => {
          // Si ha habido un error, lo mostramos y rechazamos la promesa.
          console.error(chalk.red(`Error al eliminar la carta: ${err}`));
          reject(`Error al eliminar la carta: ${err}`);
        });
    });
  }
  ...
  public listCards(): void {
    ...
    this.collection.forEach((card) => {
      ...
      if (card.líneaTipo === "Criatura" && card.fuerzaResistencia) {
        console.log(
          `Fuerza/Resistencia: ${card.fuerzaResistencia[0]}/${card.fuerzaResistencia[1]}`,
        );
      }
